...
export async function loadMarketplace(owner:string){
 ...
 const listings:Listing[]=(l.data||[]).filter(x=>listingSchema.safeParse(x.data).success).map(x=>({...listingSchema.parse(x.data),id:x.id,own:x.owner_id===owner,saved:false}));
